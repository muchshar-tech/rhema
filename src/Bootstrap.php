...
use Rhema\Common\Traits\Requester;
use Rhema\Common\Utils\Errors;
use Rhema\Config\Classes;
use Rhema\Config\I18n;
use Rhema\Config\Requirements;
...
final class Bootstrap extends Base {
	...
	public function initClasses() {
		$this->class_list = \apply_filters( 'rhema_initialized_classes', $this->class_list );
		foreach ( $this->class_list as $class ) {
			try {
				...
				\do_action( 'rhema_class_initialize_failed', $err, $class );
				Errors::wpDie(
					...
					__( 'Plugin initialize failed', 'rhema' ),
					__FILE__, $err
				);
			}
		}
	...
	public function getClasses( string $namespace ): array {
		...
		// Errors::writeLog(
		//    [
		//        'title'   => __( 'Rhema classes are not being loaded by Composer\'s Autoloader' ),
