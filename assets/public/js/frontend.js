...
var generateRandomlyChapter = function generateRandomlyChapter(assignSchema) {
  ...
  var bookData = [].concat(_toConsumableArray(data.books.old), _toConsumableArray(data.books.new));
  var bookIndexRandomly = Math.floor(Math.random() * bookData.length) + 1;
  var maxChapterNumberOfCurrentBook = (0,lodash__WEBPACK_IMPORTED_MODULE_1__.max)(Object.keys(chapterVerseInfo[bookIndexRandomly]).map(function (num) {
    return Number(num);
  }));
