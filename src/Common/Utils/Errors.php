...
class Errors {
		...
		if ( $message ) {
			...
			$source = $source ? '<code>' .
				sprintf(  /* translators: %s: file path */
					__( 'Error source: %s', 'rhema' ), $source
				) . '</code><BR><BR>' : '';
	...
	public static function pluginDie( $message = '', $subtitle = '', $source = '', $exception = '', $title = '' ) {
		if ( $message ) {
			...
			global $rhema_die_notice;
			$rhema_die_notice = $error;
		...
		add_action( 'admin_init',
			static function () {
				deactivate_plugins( plugin_basename( RHEMA_PLUGIN_FILE ) ); // phpcs:disable ImportDetection.Imports.RequireImports.Symbol -- this constant is global
			}
		);
	}
